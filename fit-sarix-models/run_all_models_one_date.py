...
def save_forecast_files(locations, forecast_date, hosp_pred_qs, case_pred_qs, q_levels, model_name):
    pred_df_by_loc = [
        construct_forecast_df(location,
                              forecast_date,
                              hosp_pred_qs[:, i, :],
                              q_levels,
                              ' day ahead inc hosp') \
            for i, location in enumerate(locations) ]
    pred_df = pd.concat(pred_df_by_loc, axis=0)
    
    # save predictions
    model_dir = Path("weekly-submission/sarix-forecasts/hosps/") / model_name
    ...
    if case_pred_qs is not None:
        ...
        # save predictions
        model_dir = Path("weekly-submission/sarix-forecasts/cases/") / model_name
        model_dir.mkdir(mode=0o775, parents=True, exist_ok=True)
        file_path = model_dir / f"{forecast_date}-{model_name}.csv"
        pred_df.to_csv(file_path, index = False)
...
def fit_sarix_variation(covariates, smooth_covariates, transform, p, d, P, D, pooling, forecast_date):
    if covariates == 'none':
    ...
    state_info['state_pop100k'] = (state_info['population'] / 100000.).values
    ...
    xy = data[modeled_vars] \
      .dropna() \
      .values
    xy = xy.reshape((len(data.location.unique()),
                     -1,
                     len(modeled_vars)))
    ...
    state_pop100k = [state_info[state_info.location == loc].state_pop100k.values[0] \
        for loc in data.location.unique()]
    state_pop100k = np.array(state_pop100k).reshape(1, -1, 1)
    ...
    save_forecast_files(locations=data.location.unique(),
...
if __name__ == "__main__":
    ...
    file_paths = [
        Path("weekly-submission/sarix-forecasts/hosps/") / model_name / f"{forecast_date}-{model_name}.csv" \
